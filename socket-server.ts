...
const userIdToData = new Map<string, { roomId: string; participant: Participant; graceTimer?: NodeJS.Timeout }>();
  ...
  for (const [sid, uid] of socketToUserId) {
    if (uid === userId) {
      ...
      if (io) {
        ...
        if (sock) sock.leave(record.roomId);
      }
    }
  }
...
function removeParticipantFromRoom(roomId: string, userId: string, io: SocketIOServer, notify = true): void {
  ...
  if (!room) return;
  const record = userIdToData.get(userId);
  const leftParticipant = record?.participant || room.participants.find((p) => p.userId === userId);
  ...
  if (notify) {
    const systemMessage: ChatMessage = {
      id: uuidv4(), senderId: 'system', senderName: 'System',
      content: `${displayName} left the party`, timestamp: new Date(), type: 'system',
    };
  ...
  if (notify) io.to(roomId).emit('participant-left', userId);
  if (room.participants.length === 0) room.emptySince = Date.now();
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', ({ roomId: customRoomId, userId, displayName }) => {
    ...
    if (customRoomId && typeof customRoomId === 'string') {
      ...
      if (!sanitizedRoomId || sanitizedRoomId.length < 2) return socket.emit('room-error', { message: 'Room name must be at least 2 characters' });
      if (rooms.has(sanitizedRoomId)) return socket.emit('room-error', { message: 'Room name already exists. Please choose another.' });
    ...
    const participant: Participant = {
      id: socket.id, userId, displayName: sanitizedName, isHost: true,
      isMuted: false, hasVideo: true, disconnected: false, joinedAt: new Date(),
    };
    const room: Room = {
    ...
    socket.emit('room-created', { room: { ...room }, participant, participants: room.participants });
  });

  socket.on('join-room', ({ roomId, displayName, userId }) => {
    if (isRateLimited(socket.id)) return;
    if (!roomId || typeof roomId !== 'string' || !displayName || typeof displayName !== 'string') return socket.emit('room-error', { message: 'Room ID and display name are required' });
    if (!userId || typeof userId !== 'string') return socket.emit('room-error', { message: 'User ID is required' });
    ...
    if (existingInRoom) {
      ...
      room.participants = room.participants.map((p) => p.userId === userId ? participant : p);
      socketToRoom.set(socket.id, normalizedRoomId);
      socketToUserId.set(socket.id, userId);
      socket.join(normalizedRoomId);
      io!.to(normalizedRoomId).emit('participant-updated', participant);
      socket.emit('room-joined', { room: { ...room, participants: room.participants }, participant, participants: room.participants });
      socket.emit('video-state-updated', room.videoState);
      console.log(`[GRACE] ${sanitizedName} reconnected successfully`);
      return;
    }

    if (room.participants.length >= 6) return socket.emit('room-error', { message: 'Room is full (max 6 participants)' });

    const participant: Participant = {
      id: socket.id, userId, displayName: sanitizedName, isHost: false,
      isMuted: false, hasVideo: true, disconnected: false, joinedAt: new Date(),
    };
    ...
    socket.emit('room-joined', { room: { ...room }, participant, participants: room.participants });
    socket.to(normalizedRoomId).emit('participant-joined', participant);

    const systemMessage: ChatMessage = {
      id: uuidv4(), senderId: 'system', senderName: 'System',
      content: `${sanitizedName} joined the party`, timestamp: new Date(), type: 'system',
    };
  ...
  socket.on('leave-room', () => {
    ...
    if (userId && roomId) removeParticipantFromRoom(roomId, userId, io!, true);
  });
  ...
  socket.on('disconnect', () => {
    ...
    record.graceTimer = setTimeout(() => {
      ...
      if (!currentRecord || currentRecord.graceTimer === undefined) return;
  ...
  socket.on('chat-message', (content: string) => {
    ...
    const message: ChatMessage = {
      id: uuidv4(), senderId: socket.id, senderName: record.participant.displayName,
      content: sanitized, timestamp: new Date(), type: 'message',
    };
  ...
  socket.on('mute-status', (isMuted: boolean) => {
    ...
    const room = rooms.get(roomId);
    if (room) { const p = room.participants.find((pp) => pp.userId === userId); if (p) p.isMuted = isMuted; }
    io!.to(roomId).emit('participant-updated', record.participant);
  });
  ...
  socket.on('video-status', (hasVideo: boolean) => {
    ...
    const room = rooms.get(roomId);
    if (room) { const p = room.participants.find((pp) => pp.userId === userId); if (p) p.hasVideo = hasVideo; }
    io!.to(roomId).emit('participant-updated', record.participant);
  });
